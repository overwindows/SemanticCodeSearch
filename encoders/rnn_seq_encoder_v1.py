...
class RNNEncoder_V1(SeqEncoder):
    @classmethod
    def get_default_hyperparameters(cls) -> Dict[str, Any]:
        encoder_hypers = {'rnn_num_layers': 2,
                          'rnn_hidden_dim': 64,
                          'rnn_cell_type': 'CUDNN_LSTM',  # One of [LSTM, GRU, RNN]
    ...
    def _encode_with_rnn(self,
        ...
        if not self.get_hyper('rnn_is_bidirectional'):
            ...
            if cell_type == 'lstm' or cell_type == 'cudnn_lstm': 
    ...
    def make_model(self, is_train: bool=False) -> tf.Tensor:
        with tf.variable_scope("rnn_encoder"):
            ...
            trans_seq_tokens_embeddings = tf.transpose(seq_tokens_embeddings,[1,0,2])
            lstm = tf.contrib.cudnn_rnn.CudnnLSTM(1,64)

            rnn_token_embeddings, rnn_final_state = lstm(trans_seq_tokens_embeddings)
